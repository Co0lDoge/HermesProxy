...
namespace HermesProxy.World.Server.Packets
{
    ...
    class EnterEncryptedModeWotlk : ServerPacket
    {
        byte[] EncryptionKey;
        bool Enabled;
        ...
        static EnterEncryptedModeWotlk()
        {
            expandedPrivateKey = Ed25519.ExpandedPrivateKeyFromSeed(EnterEncryptedModePrivateKey);
        }

        public EnterEncryptedModeWotlk(byte[] encryptionKey, bool enabled) : base(Opcode.SMSG_ENTER_ENCRYPTED_MODE)
        {
            EncryptionKey = encryptionKey;
            Enabled = enabled;
        }
