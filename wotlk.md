...
| Toy Box — learn from bags + use from journal | ❓ | ❓ | ✅ | **AzerothCore 2026-08-27:** 3.4.3 `CMSG_ADD_TOY` (12953) / `CMSG_USE_TOY` (12954) were unmapped. Learn persists in account metadata, ownership is `ActivePlayerData.Toys` (Wrathion bit 9) + `SMSG_ACCOUNT_TOY_UPDATE`. First bag copy is hidden from the client (AC still holds it so Use can forward `CMSG_USE_ITEM`). HasFanfare left off — 3.4.3 ToyBox hides the icon for that bit (model scene 253). First learn plays WotLK spell 55884 (Learning). Play-tested Orb of Deception (1973), Picnic Basket (32566), Fishing Chair (33223): live journal + Already known, grid icon, Learning anim, Use from Collections → Toys. Relog keeps the journal. TC / cMangos untested. |
