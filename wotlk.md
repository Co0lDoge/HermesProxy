...
| Quest pickup + completion | ✅ | ✅ | ⚠️ | gossip dialogs render fully **AzerothCore 2026-08-24:** title click no longer takes the quest (only Accept is forwarded). Full turn-in verified on *A Refugee's Quandary* (3361): gossip `?` → RequestItems → Continue → OfferReward → Complete Quest. Window closes; NPC gossip does not reopen. **Cancel on RequestItems / OfferReward still does not close the frame.** |
