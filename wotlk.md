...
| Toy Box — learn from bags + use from journal | ❓ | ❓ | ✅ | **AzerothCore 2026-08-27 / 28 (#170):** 3.4.3 `CMSG_ADD_TOY` (12953) / `CMSG_USE_TOY` (12954) were unmapped. Learn persists in account metadata (native 3.4.3 toys are account-wide). Ownership is `ActivePlayerData.Toys` (Wrathion bit 9) + `SMSG_ACCOUNT_TOY_UPDATE`. The bag item stays visible — hide/consume-on-learn bricked equippable toys on a 3.3.5a backend. Use forwards `CMSG_USE_ITEM` against the copy this character still carries (equipment first, then bags). Missing item → `CastFailed ItemNotFound`; equippable but not worn → `CastFailed EquippedItem`. HasFanfare left off. First learn plays WotLK spell 55884 (Learning). Journal is account-wide; use still needs the 3.3.5a item on this character. |
